...
MODULE_DIR = ROOT / "web" / "src" / "modules" / "finance" / "accounting"
...
def apply_crm_accounting() -> int:
    ...
    app = app.replace("if(a==='crm-ref-accounting-tab'){state.crmRefAccountingTab=t.dataset.tab;renderCurrentWithoutReset();return;}", "")
    ...
    required = ["ValtrenAccountingCore","state.crmAccounting","function crmAccountingPage()","function crmAccountingDreView","function crmAccountingEntriesView","function crmAccountingClassificationsView","function crmAccountingOpenDrill","state.crmFinancialTransactions","crmFinanceService()","Competência","Caixa","Receita Bruta","Resultado Final"]
    ...
    forbidden_legacy = ["P&L Empresa","P&L Projetos","P&L Artistas","P&L por Projeto","P&L por Artista","Artist P&L","Project P&L"]
    ...
    print("Financeiro → Contabilidade materializado a partir de web/src/modules/finance/accounting; P&L legado neutralizado e sidebar preservado.")
    return 1
