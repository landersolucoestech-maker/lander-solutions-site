...
if(process.argv.includes('--materialized')){
  ...
  const replacements=[[oldParticipation,newParticipation,'Participações'],[oldPayout,newPayout,'Repasses'],[oldAdministration,newAdministration,'Administração']];
  for(const [oldAssertion,newAssertion,label] of replacements){
    const occurrences=source.split(oldAssertion).length-1;
    if(occurrences!==1)throw new Error(`Assertion materializada obsoleta de ${label} esperada exatamente 1 vez em Legal Contracts UI; encontrada ${occurrences}`);
