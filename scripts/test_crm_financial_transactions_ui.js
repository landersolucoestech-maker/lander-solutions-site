...
test('13 menu de ações final contém somente Ver, Editar e Excluir',()=>{const start=presentation.indexOf('class="crm-fin-row-actions"'),end=presentation.indexOf('</td></tr>',start),block=presentation.slice(start,end);assert(start>=0&&end>start);for(const label of ['>Ver</button>','>Editar</button>','>Excluir</button>'])assert(block.includes(label));for(const forbidden of ['Lançar','Conciliar','Restaurar','Ratear','Correspondência','Ver detalhes'])assert(!block.includes(forbidden));assert.strictEqual((block.match(/<button\b/g)||[]).length,3);});

if(process.argv.includes('--materialized')){
  const app=fs.readFileSync(path.resolve(__dirname,'..','app.js'),'utf8');
  test('14 UI operacional final está no bundle',()=>{for(const marker of ['crm-fin-bulk-counterparty','crm-fin-rule-form','crm-fin-nature','crm-fin-column-config','crmFinanceSignedMoney'])assert(app.includes(marker));});
  test('15 rota publicada continua única em Financeiro → Transações',()=>{assert(app.includes("if(path==='/crm/financeiro')return crmTransactionsPage();"));});
  test('16 apresentação materializada possui apenas a coluna monetária Valor',()=>{const start=app.indexOf('// VALTREN FINANCIAL TRANSACTIONS PRESENTATION'),end=app.indexOf('// VALTREN FINANCIAL TRANSACTIONS END',start),block=app.slice(start,end);assert(start>=0&&end>start);assert.strictEqual((block.match(/<th class="right">Valor<\/th>/g)||[]).length,1);assert(!block.includes('<th class="right">Saída</th>'));assert(!block.includes('<th class="right">Entrada</th>'));});
}
