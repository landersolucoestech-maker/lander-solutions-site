...
EXPECTED={
 'grossRevenue':300000,'deductions':30000,'netRevenue':270000,'directCosts':90000,
 'operatingExpenses':60000,'operatingResult':120000,'thirdPartyParticipation':24000,'valtrenResult':96000,
}
...
def main():
 ...
 try:
  ...
  WebDriverWait(d,20).until(lambda x:x.execute_script('return !!window.__VALTREN_MOCK_MODE__'))
  time.sleep(1)
  ...
  calcs=d.execute_script("return (window.crmParticipationService?.().data.calculations||[]).filter(x=>x.id==='mock_participation_a'||x.id==='mock_participation_b').map(x=>({id:x.id,base:x.distributableBase,amount:x.participationAmount,status:x.calculationStatus,consistency:x.consistencyStatus,workflow:x.workflowStatus}))")
  evidence['participations']=calcs
  by={x['id']:x for x in calcs}
  ...
  keys=d.execute_script('return Object.keys(localStorage)');evidence['storageKeys']=keys
  bad=[x for x in keys if not x.startswith('valtren:mock:')]
  ...
  logs=d.get_log('browser');errors=[x for x in logs if x.get('level')=='SEVERE'];evidence['consoleSevere']=errors
  if errors: failures.append('Console SEVERE: '+json.dumps(errors,ensure_ascii=False)[:2000])
  d.save_screenshot(str(out/'dashboard-mock-1440.png'))
  evidence['url']=d.current_url
 except Exception as e:
  failures.append(f'BOOTSTRAP: {type(e).__name__}: {e}')
  try:d.save_screenshot(str(out/'dashboard-mock-bootstrap-failure.png'))
  except:pass
 finally:d.quit()
