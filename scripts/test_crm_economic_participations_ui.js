...
const __pathReplacements=[
  ["path.resolve(__dirname,'crm_economic_participations_browser.js')","path.resolve(__dirname,'..','web','src','modules','finance','participations','browser.js')"],
  ["path.resolve(__dirname,'crm_economic_participations.css')","path.resolve(__dirname,'..','web','src','modules','finance','participations','styles.css')"]
];
for(const [legacyPath,canonicalPath] of __pathReplacements){
  const occurrences=__partSource.split(legacyPath).length-1;
  if(occurrences!==1)throw new Error(`Referência legacy de Participações UI esperada exatamente 1 vez: ${legacyPath}; encontrada ${occurrences}`);
  __partSource=__partSource.replace(legacyPath,canonicalPath);
}
