...
def apply_crm_economic_participations() -> int:
    ...
    required = ["ValtrenParticipationCore","state.crmEconomicParticipations","function crmEconomicParticipationsPage()","function crmParticipationService()","function crmParticipationObligationsFeed(filters={})","Calcular Participações","Base de Cálculo","Base Distribuível","Memória de Cálculo","Conflito contratual de vigência.","sourceSnapshotHash","ruleSnapshotHash","crmContractEconomicRulesFeed","crmContractResolveEconomicRuleForPeriod"]
    ...
    owners = ["if(path==='/crm/financeiro')return crmTransactionsPage();","if(path==='/crm/financeiro/accounting')return crmAccountingPage();","if(path==='/crm/financeiro/notas-fiscais')return crmFiscalDocumentsPage();","if(path==='/crm/financeiro/rateios'){const page=crmCostAllocationsPage();","if(path==='/crm/juridico/contratos')return crmLegalContractsPage();","if(path==='/crm/juridico/contratos/templates')return crmLegalTemplatesPage();","if(path==='/crm/juridico/contratos/variaveis')return crmLegalVariablesPage();"]
    ...
    forbidden = ["createTransaction(","createPayout(","paidAmount","participationRules","partnerPercentages","revenueShareRules","productPartnerRules","shareholder","partnerOwnership","equityPercentage","sharePercentage"]
