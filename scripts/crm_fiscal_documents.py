...
CONSISTENCY_CSS = ROOT / "scripts" / "crm_fiscal_documents_consistency.css"
CACHE_VERSION = "20260826-fiscal-documents-v2"
...
def apply_crm_fiscal_documents() -> int:
    ...
    app = app.replace("if(kind==='invoice') html=crmRefInvoiceModal();", "if(kind==='invoice'){crmFiscalOpenCreate();return;}")
    ...
    route_new = "if(path==='/crm/financeiro/invoices')return crmFiscalLegacyInvoicesRoute();\n    if(path==='/crm/financeiro/notas-fiscais')return crmFiscalDocumentsPage();"
    app = app.replace(route_old, route_new)

    required = ["ValtrenFiscalCore","state.crmFiscalDocuments","function crmFiscalDocumentsPage()","function crmFiscalOpenCreate()","function crmFiscalOpenForm(direction)","function crmFiscalOpenDetail(id)","function crmFiscalAccountingFeed(filters={})","crmFinanceService()","crmCanonicalPartyService()","Criar Nota","Entrada","Saída","Status Fiscal","Status Financeiro","competenceDate","fiscal_document"]
    ...
    print("Financeiro → Notas Fiscais materializado como fonte fiscal canônica; escala visual normalizada; Invoice legado neutralizado; Transações, Contabilidade e sidebar preservados.")
    return 1
