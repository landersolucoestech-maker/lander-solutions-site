...
CACHE_VERSION = "20260825-payouts-v1"
...
def apply_crm_payouts() -> int:
    ...
    # Idempotent owner block: remove only our previous injection, never another module.
    app = re.sub(
    ...
    if old_count == 1 and new_count == 0:
        ...
        # Applying the same owner twice must be semantically idempotent.
        pass
    else:
        raise RuntimeError(
            "Rota de Repasses não está em estado canônico inequívoco: "
            f"placeholder={old_count}, handler={new_count}"
        )

    required = [
        "ValtrenPayoutCore",
        "state.crmPayouts",
        "function crmPayoutsPage()",
        ...
        "Saldo",
        "Conciliação",
        "payment.reversal_detected",
        "source_superseded_with_payments",
        "transactionLinks",
        "sourceSnapshots",
    ]
    ...
    # Repasses settles an approved obligation. It cannot own economic rules or create money movement.
    forbidden = [
        "createTransaction(",
        "createCalculation(",
        "manualParticipationRule",
        "partnerPercentage",
        "productPartnerSplit",
    ...
    print("Financeiro → Repasses materializado como camada canônica de liquidação/conciliação de Participações aprovadas; nenhuma Participação ou Transação é recriada.")
    return 1
